...
apply(gridOfparams, 1, function(params){
  ...
  load(paste0("~/Desktop/TimeChange-data/Results/syntheticData_general/limoRhyde/", fileName, ".RData"))
  ...
  perCorrectClassification <- lapply(1:6, function(i) {
    ...
    splitByWaveform <- split(unlist(selectOutput$results$FDR), ceiling(seq_along(unlist(selectOutput$results$FDR)) / 1000))
  ...
  ouputFile <- paste0("~/Desktop/TimeChange-data/Results/syntheticData_general/limoRhyde/figs/", fileName, ".png")
