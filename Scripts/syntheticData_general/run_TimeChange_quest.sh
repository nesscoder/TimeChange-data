...
cd /home/emn6548/TimeChange-data/Scripts/syntheticData_general/

Rscript processSyntheticData_general_quest.R $fileName $fileIn $fileOut
EOJ
`
# print out the job id for reference later
echo "JobID = ${JOB} for parameters $fileName submitted on `date`"
done < masterFilePath_timeChange_general.txt
exit
