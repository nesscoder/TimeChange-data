...
cd /home/emn6548/TimeChange-data/Scripts/syntheticData_dphase/

Rscript processSyntheticData_dphase_quest.R $fileName $fileIn $fileOut
EOJ
`
# print out the job id for reference later
echo "JobID = ${JOB} for parameters $fileName submitted on `date`"
done < masterFilePath_timeChange_dphase.txt
exit
